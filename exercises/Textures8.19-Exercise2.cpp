...
int main()
{
	...
	*/ 
	...
	// Free the Binded vertex array object
	glBindVertexArray(0);

	/********************************************************
	 * Creating another cube object
	 ********************************************************/
	...
	{		
		...
		glm::mat4 otherTrans = glm::mat4(1.0f);
		// Translate
		otherTrans = glm::translate(otherTrans, glm::vec3(-0.5f, 0.5f, 0.0f));
		float scaleFactor = (std::sin(glfwGetTime()) / 2.0f) + 0.5f;
		otherTrans = glm::scale(otherTrans,  glm::vec3(scaleFactor, scaleFactor, 1.0f));
		glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(otherTrans));
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
